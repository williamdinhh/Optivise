...
export async function fetchStatsigEvents(
  ...
  try {
    ...
    console.log(
      "ℹ️  Statsig events API not available - using local event tracking instead"
    );
    console.log(
      "💡 To get real Statsig data, use Statsig webhooks or data export features"
    );
...
export async function getRealStatsigMetrics() {
  ...
  if (events.length === 0) {
    console.warn(
      "⚠️  No events found in Statsig. Make sure capture is running and events are being sent."
    );
    return null;
  }
